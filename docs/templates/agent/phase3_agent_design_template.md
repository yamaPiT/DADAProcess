...
> 【AIへの指示】`UNIT-EX`・`ADR-EX` / `ADR-EX2`（記入例）は理解したら削除すること。文章は `docs/guidelines/asdoq_writing_rules.md`（執筆ルール12箇条）に従って書くこと。
> 【AIへの指示】レイヤーの振り分けとポカヨケ設計基準は `docs/guidelines/agent_design_principles.md` の第3章・第5章に従うこと。**三原則は必守。5つの設計パターンは3.1節で5行とも採用／不採用と理由を書く（採用は任意。空欄禁止）。** 手順は同ガイドライン第2.4節。
...
**設計パターンの吟味**（`agent_design_principles.md` 第2.2節。**5行とも埋める。採用は任意。空欄禁止。** SW105の2.5節を転記し、設計として確定する）

| パターン | 採用 / 不採用 | 適用箇所 | 理由 |
| :--- | :--- | :--- | :--- |
| Prompt Chaining（プロンプトチェーニング） | | （例: 朱入れ文書を次の執筆の入力にする） | |
| Routing（ルーティング） | | （例: 入力の種類でスキルを振り分ける） | |
| Parallelization（並列化） | | （例: 独立な章の並列生成） | |
| Orchestrator-Workers（オーケストレーター・ワーカー） | | （例: 調整役が執筆役と評価役を呼ぶ） | |
| Evaluator-Optimizer（エバリュエーター・オプティマイザー） | | （例: 執筆役と評価役を別スキルにする） | |
...
**スキル分割方針**: （SW105の2.3節の役割を1役割＝1スキル＝1 UNIT で落とす。分割しない役割はADRに理由を残す。Evaluator-Optimizer 採用時は Generator と Evaluator を同一UNITにしない。発動条件が重複しないことを説明する）
...
> 【AIへの指示】`- **公開インターフェース**:` の書き方は、レイヤーによって変える。自然言語レイヤーは「発動条件（description）／入力（読むファイル）／出力（書くファイル）／思考過程の出力／出力形式／呼び出すツール」を書く。プログラムレイヤーは「コマンド行／引数／標準出力／終了コード」を書く。
...
- **対応要求**: REQ-EX
- **公開インターフェース**: 発動条件=「会議メモから議事録を作る依頼を受けたとき」（frontmatterの `description` に記載） / 入力=利用者が投入した会議メモ本文、`references/minutes_template.md`、朱入れ文書があればそのパス / 出力=`docs/minutes/YYYY-MM-DD.md` / 思考過程の出力=抽出方針・決定事項の候補・次に書く節を応答に出す / 出力形式=Markdown。見出し「決定事項」「TODO」「次回議題」 / 呼び出すツール=UNIT-EX2（書式検証）
- **依存先**: UNIT-EX2（書式検証ツール。終了コード0を確認してから完了報告する）
- **検証条件**: 必須見出し3つを含む議事録が生成され、UNIT-EX2の終了コードが0であること。
- **適用するポカヨケ**: 思考過程の強制／ルート固定パス（出力先を literal で固定）／statusコード制御（UNIT-EX2の終了コードで分岐）／完了条件のチェックリスト化
...
- **成果物パス**: （生成するファイルのパス）
- **対応要求**: （REQ-ID列挙）
- **公開インターフェース**: （レイヤーに応じた書き方。上記の記入例に倣う。自然言語は思考過程の出力と出力形式を含める）
- **依存先**: （UNIT-ID。なければ「なし」）
- **検証条件**: （単体で合否判定できる条件）
...
#### ADR-EX2: （記入例）議事録の執筆と評価を別スキルにする
- **背景**: 1スキルに抽出・執筆・自己採点を詰めると、評価が甘くなり発動条件も濁る。
- **決定**: Generator（執筆）と Evaluator（評価）と Orchestrator（調整）を別UNITにする。
- **理由**: 三原則の Simplicity。Evaluator-Optimizer と Orchestrator-Workers を採用する（SW105の2.5節）。
- **捨てた選択肢**: 1スキルで自己校正する（簡単な依頼なら採用しうるが、本課題は評価基準が明確なため分割する）。
