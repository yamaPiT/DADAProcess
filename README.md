...
青枠は**決定論的なスクリプトによる機械チェック**（`tools/dada_check.py`）で、AIの推論に頼らずID対応漏れや曖昧語を検出する工程です。緑枠はドキュメント、灰色の破線枠はAI内に隠蔽される工程です。
...
graph TD
    ...
    subgraph P1 ["Phase 1: 要求定義"]
        ...
        ReqChk -- 形式的欠陥を修正 --> Req
        ReqChk --> ReqRev["自己レビュー - Self-Correction"]
        ReqRev -- 修正・洗練 --> Req
    ...
    subgraph P2 ["Phase 2: 総合テスト仕様策定"]
        ...
        TestChk --> TestRev["自己レビュー - Self-Correction"]
    ...
    subgraph P3 ["Phase 3: アーキテクチャ設計"]
        ...
        ArchChk --> ArchRev["自己レビュー - Self-Correction"]
        ArchRev -- 修正・洗練 --> Arch
    ...
    subgraph P4 ["Phase 4: 実装・デバッグ"]
        ...
        Impl --- ProgDoc[("ソースコード")]
        Impl --- Ledger[("実装進捗台帳 impl_progress.md")]
        Impl --> ImplLoop["Unit Test Generator - 自律デバッグループ"]
        ImplLoop -- テスト失敗 --> Impl
        ImplLoop -- オールグリーン --> CodeChk["機械チェック - 未実装ユニット・テスト無効化の検出"]
        CodeChk -- 欠落を修正 --> Impl
        CodeChk --> CodeRev["自己レビュー - Self-Correction"]
    ...
    class ReqDoc,ArchDoc,ProgDoc,TestDoc,Ledger doc;
...
| [`docs/process/`](docs/process/) | **プロセス状態・文脈管理** | フェーズ間の引き継ぎサマリー（`last_phase_summary.md`）、Phase 4の実装進捗台帳（`impl_progress.md`）、機械チェック結果（`check_report.md`）、Phase 5評価ガイド（`phase5_evaluation_guide.md`・任意）。開発プロセスの状態を**AIの記憶の外に**記録・復元し、アテンション・リセットと中断からの再開を支えます。 |
...
| [`tools/`](tools/) | **機械チェックツール** | 決定論的な検証スクリプト `dada_check.py`。ID対応（REQ↔TC↔UNIT）の照合、曖昧語・表の空欄の検出、未実装ユニットの検出を、AIの推論ではなくコマンド実行で行います。Python標準ライブラリのみで動作し、追加インストールは不要です。 |
